...
class MainPageUseCase @Inject constructor(
    ...
    fun getFeed(searchQuery: String? = null, lastDateTime: Long? = null, postType: Int? = null): Flow<BaseResult<List<PostDto>>> {
        return flow {
            // TODO : Get Posts from cache

            val feed = postRepository.getFeed(searchQuery = searchQuery, dateTime = lastDateTime, postType = postType)
